...
class BertLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, model_url, bert_model='base', num_fine_tune_layers=12, output_mode="sequence", **kwargs):
        """Constructor for BERT Layer.

        Args:
            model_url (string): Tensorflow Hub Model url. Default should be "https://tfhub.dev/google/bert_uncased_L-12_H-768_A-12/1"
            bert_model (string): Determines the model type for the BERT module (base or large). Default is base.
            num_fine_tune_layers (int): Int between 1 and 12 (bert base), or 1 and 24 (bert large),
                    determines how many bert layers are fine tuned
            output_mode (string):
